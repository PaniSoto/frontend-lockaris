  ...
  const handleLogout = () => {
    Alert.alert('Cerrar Sesión', '¿Estás seguro de que quieres salir de Lockaris?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: async () => {
          await authService.logout(); // Borra el token de SecureStore
          router.replace('/'); // Te manda de vuelta al Index
        },
      },
    ]);
  };
    ...
    <View className="flex-1 bg-slate-50">
      ...
      <View className="px-6">
        <View className="overflow-hidden rounded-[30px] border border-slate-100 bg-white shadow-sm">
          {/* Opción de Perfil (Solo visual por ahora) */}
          <TouchableOpacity className="flex-row items-center border-b border-slate-50 p-5">
