...
export default function AddCredentialModal({ isOpen, onClose, onSave, itemType }) {
  // Estado interno: el formulario nace y muere con el modal
  const initialState = {
    serviceName: '', 
    username: '', 
    password: '', 
  ...
  // Limpiar el formulario cada vez que el modal se abre/cierra
  useEffect(() => {
    if (isOpen) setLocalForm(initialState);
  }, [isOpen]);

  const handleLocalSave = () => {
    // Pasamos los datos al padre (VaultScreen) para que haga la persistencia
    onSave(localForm);
  };
      ...
      <View className="flex-1 justify-end bg-black/50">
        ...
        >
          <View className="mb-6 flex-row items-center justify-between">
            <View>
              <Text className="text-xs font-bold text-blue-500 uppercase tracking-widest">Crear Nuevo</Text>
            ...
            renderItem={() => (
              <View className="gap-y-4 pb-20">
                ...
                {itemType === 'CARD' && (
                  <>
                    <View>
                      <Text className="mb-1 ml-1 font-medium text-slate-500">Nombre en Tarjeta</Text>
                      <TextInput 
                    ...
                    <View className="flex-row gap-x-4">
                      <View className="flex-1">
                        ...
                        <TextInput className="rounded-2xl bg-slate-100 p-4" placeholder="MM/AA" value={localForm.expiryDate} onChangeText={(t) => setLocalForm({ ...localForm, expiryDate: t })} />
                      </View>
                      <View className="flex-1">
                        <Text className="mb-1 ml-1 font-medium text-slate-500">CVV</Text>
                        <TextInput className="rounded-2xl bg-slate-100 p-4" secureTextEntry keyboardType="numeric" value={localForm.cvv} onChangeText={(t) => setLocalForm({ ...localForm, cvv: t })} />
                      </View>
                    </View>
                  </>
                )}
                ...
                >
                  <Text className="text-center text-lg font-bold text-white">Guardar en Bóveda</Text>
                </TouchableOpacity>
              </View>
            )}
          />
